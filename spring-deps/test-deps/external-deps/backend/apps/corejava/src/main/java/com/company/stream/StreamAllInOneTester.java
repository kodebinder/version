...
//      Optional<Employee> employeeWithHighestSalary2 = employees.stream().collect(Collectors.maxBy(Comparator.comparingDouble(Employee::getSalary)));
        ...
        System.out.println("\nDisplay details of youngest male employee in com.company.stringhandling.Product Development department");
        Optional<Employee> optionalYoungestEmployeeInProductDevelopment = employees.stream().filter(e -> "Male".equals(e.getGender()) && "com.company.stringhandling.Product Development".equals(e.getDepartment())).min(Comparator.comparingInt(Employee::getAge));
